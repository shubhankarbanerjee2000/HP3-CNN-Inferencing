					 ...
					 float * data_col, float * data_out, int bs)
{
	...
	GEMM_manual(data_col, data_ker, data_out, m, n, k, bs, CUDA_NUM_THREADS);
}

__global__ void matrixMul(const float *A, float int *B, float *C, int m, int n, int k, int t) {
...
void GEMM_manual(const float *A, const float *B, float *C, int m, int n, int k, int t, int THREADS) {
  ...
  // Launch kernel
  matrixMul<<<blocks, threads>>>(A, B, C, m, n, k, t);
