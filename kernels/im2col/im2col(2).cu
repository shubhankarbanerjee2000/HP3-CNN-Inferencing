...
__global__ void matrixMul(const float *A,const float *B, float *C, int m, int n, int k, int batch_size, int SHMEM_SIZE) {
    ...
    for(int t = 0;t < batch_size; t++) {
        ...
        for (int i = 0; i < k; i += blockDim.x) {
          ...
          // Wait for all CUDA_NUM_THREADS to finish using current tiles before loading in new
          // ones
          __syncCUDA_NUM_THREADS();
    }
							  ...
							  const int hcol, const int wcol) 
{
	...
	// but since we are launching enough CUDA_NUM_THREADS to handle each image separately, it executes just once
	// here it is majorly prevents any extra CUDA_NUM_THREADS we launch from accessing memory
					 ...
					 float * data_col, float * data_out, int bs)
{
	...
	int SHMEM_SIZE = devProp.sharedMemPerBlock/(2*sizeof(float))
	...
	// We are going to launch bs groups of ic * hcol * wcol kernels CUDA_NUM_THREADS for im2col,
	...
	dim3 CUDA_NUM_THREADS(CUDA_NUM_CUDA_NUM_THREADS, 1, 1);
	im2col_kernel<<<blocks, CUDA_NUM_THREADS>>>(data_im, data_col, op_size, kh, kw, pad, stride, ih, iw, ic, hcol, wcol);
	...
	dim3 threads(CUDA_NUM_THREADS, CUDA_NUM_THREADS);
	dim3 blocks(BLOCKS_X, BLOCKS_Y);
  
	// Launch kernel
	matrixMul<<<blocks, threads>>>(data_col, data_ker, data_out, m, n, k, bs, SHMEM_SIZE);
}
